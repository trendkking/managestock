...
export default function JournalChartPage() {
  ...
  return (
    <div>
      ...
      <PageHeader
        title={`${stockName} 차트`}
        description={`${stockCode}${chart.chartMeta ? ` · ${chart.chartMeta.market}` : ''} · ${CHART_INITIAL_VISIBLE_BARS}봉(약 ${CHART_VISIBLE_MONTHS}개월) · Ctrl+휠/핀치 확대·축소 · 드래그 이동 · 기록 ${entries.length}건`}
        action={
