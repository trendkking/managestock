...
import { ohlcOnDate, resolvePriceDate, CHART_FETCH_MONTHS } from '@/lib/journalStockChart'
...
export default function JournalChartPage() {
  ...
  return (
    <div>
      ...
      <PageHeader
        title={`${stockName} 차트`}
        description={`${stockCode}${chart.chartMeta ? ` · ${chart.chartMeta.market}` : ''} · ${CHART_FETCH_MONTHS}개월 표시 · 휠/드래그로 이동 · 기록 ${entries.length}건`}
        action={
